...
In the code below, different types of errors can occur from lines 5-8. By
placing ``IOError`` after ``except`` on line 9, only errors regarding Input and
Output (IO) will be handled by that code. Likewise line 11 only handles
errors around converting values, and line 13 covers division by zero errors.
The last exception handling occurs on line 15. Since line 15 does not include
